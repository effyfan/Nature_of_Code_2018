// Fixing it with Chino
// replaced cities in calcDistance() with selectedObjectArr
// swap selectedObjectArr instead of cities
// make bestEver the lat and lng first, then conver it into citiesBestEver
...
let drawRoute = () => {
  if (cities.length > 0 && !modifying) {
    ...
    noFill();
    beginShape();
    for (let i = 0; i < citiesBestEver.length; i++) {
      // draw lines between cities
      vertex(citiesBestEver[i].x, citiesBestEver[i].y);
    }
    endShape();
